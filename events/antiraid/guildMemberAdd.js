...
module.exports = async (client, member) => {
    ...
    if (action.executor.id) {
        console.log("Executor ID:", action.executor.id);
        let perm = ""
        ...
        if (db.get(`bot_${guild.id}`) === true && !perm) {
            if (db.get(`botsanction_${guild.id}`) === "ban") {
                ...
                axios({
                    ...
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, il a été **ban** !`));
                    await member.kick(); // Expulser le bot
                }).catch(async () => {
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, mais il n'a pas pu être **ban** !`));
                });
            } else if (db.get(`botsanction_${guild.id}`) === "kick") {
                console.log("kick");
                guild.members.cache.get(action.executor.id).kick().then(async () => {
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, il a été **kick** !`));
                    await member.kick(); // Expulser le bot
                }).catch(async () => {
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, mais il n'a pas pu être **kick** !`));
                });
            } else if (db.get(`botsanction_${guild.id}`) === "derank") {
                console.log("derank");
                guild.members.cache.get(action.executor.id).roles.set([]).then(async () => {
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, il a été **derank** !`));
                    await member.kick(); // Expulser le bot
                }).catch(async () => {
                    if (raidlog) await raidlog.send(new MessageEmbed().setColor(color).setDescription(`<@${action.executor.id}> a invité le bot ${member}, mais il n'a pas pu être **derank** !`));
                });
            }
        }
    }
