...
if (db.get(`randombanner_${guild.id}`)) {
    setInterval(async () => {
        ...
        if (channel) {
            ...
            const bannerUrl = user.displayAvatarURL({ dynamic: true, size: 512 });
            ...
            channel.send(embed, button).then(slm => {
                console.log(`Banner envoyée dans ${guild.channels.cache.get(db.get(`randombanner_${guild.id}`)).name}`);
            }).catch(err => {});
        }
    }, interval);
}}})
