...
module.exports = {
    ...
    run: async (client, message, args, prefix, color) => {
        if (!args[0]) {
            return message.channel.send("Veuillez fournir l'ID de l'utilisateur à débannir.");
        }
            if(args[0] == "all") {
                let perm = ""
        ...
        const bannedUser = banList.find((slm) => slm.user.id === user.id);
            if (!bannedUser) {
                return message.channel.send(`Aucun membre banni trouvé pour l'ID : \`${args[0]}\``);
            }
