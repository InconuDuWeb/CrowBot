    ...
    run: async (client, message, args, prefix, color) => {
        ...
        if (
            ...
            function mutetime(user, time, authorcooldown, muterole) {
                const previousRoles = user.roles.cache.map(role => role.id);  // Enregistrer les rôles précédents
                user.roles.set([muterole.id]).then(() => {
                    ...
                    setTimeout(() => {
                        ...
                        previousRoles.forEach(roleID => {
                            ...
                            if (role) {
                                user.roles.add(role);  // Réaffecter les rôles précédents
                            }
                        });
                        muterole.delete().catch(console.error);
                    }, time);
                    setTimeout(() => {
            ...
            function mute(user, authorcooldown, muterole) {
                ...
                const previousRoles = user.roles.cache.map(role => role.id);
                db.set(`previousRoles_${message.guild.id}_${user.id}`, previousRoles);
                user.roles.set([muterole.id]).then(r => {
                    authorcooldown.limit++;
                    setTimeout(() => {
                        authorcooldown.limit = authorcooldown.limit - 1;
                    }, 120000);
            ...
            if (args[0]) {
                ...
                if (authorcooldown.limit >= 5) return message.channel.send(`Vous avez atteint votre limite de **mute**, veuillez réessayer plus tard!`);
                if (!muterole) {
